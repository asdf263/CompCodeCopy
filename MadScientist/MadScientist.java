...
public class MadScientist {
    ...
    public static void main(String[] args) throws IOException{
        ...
        InputReader input = new InputReader(new FileReader("breedflip.in"));
        PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter("breedflip.out")));

        int N = input.nextInt();
        char[] desiredCows = new char[N];
        char[] curCows = new char[N];
        String A = input.next();
        String B = input.next();
        for(int i = 0; i < N; i++){
            desiredCows[i] = A.charAt(i);
            curCows[i] = B.charAt(i);
        }
        int solution = 0;
        boolean chainOfWrongCows = desiredCows[0] == curCows[0]? false: true; 
        for(int i = 1; i < N; i++){ 
            if(desiredCows[i] == curCows[i]){
                if(chainOfWrongCows == true){
                    solution++;
                    chainOfWrongCows = false;
                }
            } else{
                chainOfWrongCows = true;
            }
        }
