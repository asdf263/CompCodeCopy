...
public class MeasuringTraffic {
    ...
    public static void main(String[] args) throws IOException{
        ...
        InputReader input = new InputReader(new FileReader("traffic.in"));
        PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter("traffic.out")));
    
        int N = input.nextInt();
        String[] ramptype = new String[N];
        int[] minValues = new int[N];
        int[] maxValues = new int[N];

        for(int i = 0; i < N; i++){
            ramptype[i] = input.next();
            minValues[i] = input.nextInt();
            maxValues[i] = input.nextInt();
        }
        int[] solution = new int[4];
        int max = 999999999;
        int min = 0;
        for(int i = 0; i < N; i++){
            if(ramptype[i].equals("off")){
                min -= maxValues[i];
                max -= minValues[i];
            } else if(ramptype[i].equals("on")){
                min += minValues[i];
                max += maxValues[i];
            } else{
                if(maxValues[i] < max){
        ...
        for(int i = N-1; i >= 0; i--){
            if(ramptype[i].equals("on")){
                min -= maxValues[i];
                max -= minValues[i];
            } else if(ramptype[i].equals("off")){
                min += minValues[i];
                max += maxValues[i];
            } else{
                if(maxValues[i] < max){
        ...
        System.out.println(solution[2] + " " + solution[3]);
        ...
        // Step 3, output
        pw.println(min + " " + max);
        pw.println(solution[2] + " " + solution[3]);
