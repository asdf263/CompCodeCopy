...
public class SocialDistancingII {
    ...
    public static void main(String[] args) throws IOException{
        ...
        InputReader input = new InputReader(new FileReader("socdist2.in"));
        PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter("socdist2.out")));
        int N = input.nextInt();
        HashMap<Integer, Boolean> cowsMap = new HashMap<>();
        int[] cowsPos = new int[N];
        boolean[] cowsSick = new boolean[N];
        int R = 999999999;
        for(int i = 0; i < N; i++){
            cowsPos[i] = input.nextInt();
            if(input.nextInt() == 1){
                cowsSick[i] = true;
            } else{
                cowsSick[i] = false;
        ...
        for(int i = 0; i < N; i++){
            ...
            for(int j = 0; j < N; j++){
                ...
                R = Math.abs(cowsPos[i] - cowsPos[j]) - 1 >  R ? R : Math.abs(cowsPos[i] - cowsPos[j]) - 1;
                
            }
        }
        
        for(int i = 0; i < N; i++){
            cowsMap.put(cowsPos[i], cowsSick[i]);
        }
        int solution = 0;
        int beforeSickCow = -1993999999;
        for(int i = 0; i < 1000001; i++){
            if(cowsMap.get(i) == null || cowsMap.get(i) == false){
                continue;
            }

            if(i - R > beforeSickCow){
                solution++;
            }
            beforeSickCow = i;
